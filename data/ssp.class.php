        ...
        for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
            ...
            for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
                ...
                if ( isset( $column['formatter'] ) ) {
                    $row[ $column['dt'] ] = ($isJoin) ? utf8_encode(SSP::convert_htmlentities($column['formatter']( $data[$i][ $column['field'] ], $data[$i] ))) : utf8_encode(SSP::convert_htmlentities($column['formatter']( $data[$i][ $column['db'] ], $data[$i] )));
                }
                else {
                    $row[ $column['dt'] ] = ($isJoin) ? utf8_encode(SSP::convert_htmlentities($data[$i][ $columns[$j]['field'] ])) : utf8_encode(SSP::convert_htmlentities($data[$i][ $columns[$j]['db'] ]));
                }
            }
...
/*var_dump((array(
	...
	static function convert_htmlentities($data)
	{
